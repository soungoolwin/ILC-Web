<x-guest-layout>
    <div class="flex justify-center items-center min-h-screen">
        <div id="signup_main_Div" class="w-full max-w-lg bg-white shadow-xl rounded-lg p-8">
            ...
            <form method="POST" action="{{ route('register.lecturer') }}" class="space-y-4">
                @csrf

                <h3 class="form-title text-center text-2xl font-bold text-gray-800 mb-6">Lecturer Registration</h3>
                ...
                <!-- Admin ID -->
                <div>
                    <label for="admin_id" class="block text-sm font-semibold text-gray-600 mb-1">Lecturer ID</label>
                    <input type="text" id="admin_id" name="admin_id" value="{{ old('admin_id') }}"
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-blue-400 focus:outline-none shadow-sm transition"
                        placeholder="Enter your Lectuer ID" required>
                </div>
